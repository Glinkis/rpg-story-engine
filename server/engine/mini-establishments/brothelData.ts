...
export const brothelData = {
  ...
  notice: [
    ...
    `scented candles burning cheerily on a counter. The smell of ` +
      `<<print setup.brothel.brothelScents.random()>>` +
      ` wafts from where the candles burn`,

    `the soft ${randomValue([
      `red`,
      `orange`,
      `golden`,
      `dark blue`,
      ...
      `maroon`,
      `pink`,
    ])} glow from a shaded lamp in the room`,
    ...
    `a rather distasteful tapestry hanging on the wall depicting ${randomValue([
      `a rather lewd act`,
      `a woman with both breasts hanging out`,
      `a man with his bits hanging out`,
      `some sort of sexual story`,
      `the greatest sexual deeds of the brothel owner`,
    ])}`,
    ...
    `a bard clad in only his undergarments standing in one corner playing ${randomValue([
      `a sweet tune on a lute`,
      `a beat on a small wooden box`,
      `an off-key song on a lute`,
      `a bitter tune on a harp`,
      `an interesting song on a sitar`,
      `a merry tune on a flute`,
      `a quick beat on a pair of drums`,
      `a fine song on a fiddle`,
  ...
  idle: [
    ...
    `pouring over a book titled ${randomValue([
      `The Lustful Practice`,
      `Helpful Harlot Hints`,
      `From Rags to Wenches: A Pimp's Trip to the Top`,
      `Glory Holes`,
      `Venus and Adonis`,
      `The Seven Beauties`,
      `Sappho and the Other Raunchy Poets`,
      `The Complete Works of Archilochus`,
      `Poetica Erotica`,
      `50 Positions to Make Him Drop his Coin Sack`,
      `Sex and Sorcery`,
      `Toys for Boys`,
    ])}`,
  ...
  harlot: {
    create(town, brothel, base?) {
      const harlotType = randomValue(Object.keys(brothelData.harlot.type))
      ...
      const harlot = createNPC(town, {
        ...
        looks: randomValue(brothelData.harlot.looks),
        skill: randomValue(brothelData.harlot.skill),
        hasClass: false,
        profession: `harlot`,
        harlotType,
      ...
      createRelationship(town, harlot, variables.npcs[brothel.pimp.key], `employer`, `employee`)

      return harlot
    },
    readout(harlot) {
      return `This harlot is ${harlot.harlotType} called ${harlot.name}. She has ${harlot.feature} and is particularly good at ${harlot.skill}. However, she has ${harlot.physicalTrait}, which is ${harlot.flawSeverity}. She is looking to ${harlot.looks}.`
    },
    type: {
      ...
      "the bastard daughter of a noble house": {
        ...
        callbackFunction(town, npc) {
          ...
          createParentage(town, npc.family, npc, true)
        },
      },
      "a young foreigner": {
    ...
    skill: [
      ...
      `certain lewd oral activities`,
      `getting clients in the mood`,
      `juggling`,
    ],
    // However, she has __
    physicalTrait: [
