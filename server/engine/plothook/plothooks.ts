...
export const plothooks: Record<string, Plothook> = {
  ...
  "The Godfather": {
    ...
    function(town) {
      ...
      const npcProfile = profile(npc, `crime lord`)
      return `The party stops to witness a funeral procession for a ${npcProfile}. Soon after passing, the party hears an explosion and sees a gigantic smoking fireball rise into the sky a block away.`
    },
  },
  "The Cage": {
    type: [PlothookType.Event],
    function(town) {
      ...
      const npcProfile = profile(npc, `important socialite`)
      return `The party is invited to a manor by an ${npcProfile} for a potential job opportunity. When they arrive, they are told to wait in an empty room. Soon after entering the room, the party realizes they are locked in with a magical ward.`
    },
  },
  "The Metal Dog": {
    type: [PlothookType.Event],
    ...
    function(town) {
      ...
      const npcProfile = profile(npc, `disheveled girl`)
      return `As you step out of the tavern, a ${npcProfile} smashes into you. 'Wilkenson's dog is loose again!' she exclaims as she attempts to hide behind you. You turn back in the direction she came to see a metal construct resembling a dog charging toward you.`
    },
  },
  "The Pregnant Woman": {
    type: [PlothookType.Event],
    ...
    function(town) {
      ...
      const npcProfile = profile(npc, `pregnant woman`)
      return `The PCs spot a ${npcProfile} walking through the bazaar. As she passes the baker's stall she quickly grabs a loaf of bread and keeps walking, favouring one of the PCs with a wink as she goes. Is she a thief or the wife of the store owner?`
    },
  },
  ...
  "Indie": {
    ...
    function(town) {
      ...
      const npcProfile = profile(npc, `museum curator`)
      return `One member of the party is seen carrying an exotic weapon and the authority of the local museum is told about it. This ${npcProfile} has been an adventurer and masters some kind of whip weapon. He prepares an ambush to steal the exotic weapon, yelling at the PCs on sight, 'That belongs in a museum!'`
    },
  },
  "The Tourist": {
    type: [PlothookType.Event],
    function(town) {
      ...
      const npcProfile = profile(npc, `merchant`)
      return `The PCs have messed up (possibly been framed) in the eyes of the King and have been apprehended. The King will release the party, if they allow a wealthy, influential, and obese ${npcProfile} to come along on a dungeon delve, where there are sure to be many tight spaces.`
    },
  },
  "The Blacksmith Competition": {
    ...
    function(town) {
      ...
      const blacksmithProfile = profile(smithy.blacksmith)
      const smithyProfile = profile(smithy, ``, `town.buildings.smithy`)
      const npcProfile = profile(npc)
      return `Two blacksmiths, ${blacksmithProfile} of ${smithyProfile} and ${npcProfile} are in competition to create better melee weapons for the King's army and only one can win the contract. ${npc.firstName} approaches the PCs to try the weapons and plead their virtues to the King. He also claims his competitors are playing dirty.`
    },
  },
  "Kindergarten Magic": {
    type: [PlothookType.Event],
    ...
    function(town) {
      ...
      return `Street urchins are rumored to have special powers. A ${npcProfile} of ${factionProfile} approaches your party to investigate. In reality, they are learning magic from an unknown source. The children have less inhibition and magical control, but have much more mana and capability of replenishing mana, making them dangerous magic users.`
    },
  },
  "The Crystal Ball": {
    type: [PlothookType.Event],
  ...
  "The Werewolf Maiden": {
    ...
    function(town) {
      ...
      return `The PCs hear shrieks from a dark alley where a ${npcProfile} just went. Under the full moon, she is painfully turning into a werewolf for the first time.`
    },
  },
  ...
  "The Thieves Fun": {
    ...
    function(town) {
      ...
      const factionProfile = profile(faction, `guild of thieves`, `town.factions`)
      return `PCs are marked by a ${factionProfile}, for a contest. Low-skill, would-be guild members keep targeting the party with attempts on their health or goods.`
    },
  },
  "The Spoiled Kid": {
    type: [PlothookType.Event],
    function(town) {
      ...
      return `The ${npcProfile} of a noble finds a PC has something he just *has to have*. The noble's staff does what it can to acquire it, by nearly any means necessary.`
    },
  },
  "The Bad Architect": {
  ...
  "The Corrupt Guards": {
    ...
    function(town) {
      ...
      return `Down the dimly lit alley, two guards can be seen beating a kneeling man. ${npcProfile} rips a coin purse from the kneeling man's belt and says, 'Don't be late next month, old man.'`
    },
  },
  "The Umber Hulk": {
    type: [PlothookType.Event],
  ...
  "Hubert": {
    ...
    function(town) {
      const faction = factionsForType(town, `type`, `wizards`)
      return `${`‘Missing: a large turtle named Hubert who has escaped from the research department at ` +
        `<<link "`}${JSON.stringify(faction.name)}">><<set $selected to {faction: ${JSON.stringify(
        faction
  ...
  "Big Chickens": {
    ...
    function(town) {
      ...
      const npcProfile = profile(npc, `witch`)
      return `Chicken Wrangler Needed! Some jerks smashed my coop and now 100 chickens are loose. <<money 100>>/chicken to return them, no questions asked.’ (Poster is True Neutral ${npcProfile} that lives in a ${cabin.tippyWord} in old haunted forest; all the chickens have been enlarged.)`
    },
  },
  "Guard The Garlic": {
    type: [PlothookType.Paper],
    ...
    function(town) {
      ...
      const npcProfile = profile(npc, `I`)
      return `${npcProfile}need men to form a small honour garrison for the banquet I am holding in my manor. Important people are involved, so discretion is mandatory. <<money 20000>> per person, payed upfront. No questions asked. (All the guests are vampires.)`
    },
  },
  "Books": {
    type: [PlothookType.Paper],
    function(town) {
      ...
      return `${npcProfile} book store has a shipment of rare books coming in and needs people to help unload them. Will pay 100 gold per person and not responsible for any injuries incurred on the job. (Caution: dangerous magic and pet books around.)`
    },
  },
  "Rat Exterminator": {
    type: [PlothookType.Paper],
    function(town) {
      ...
      return `Exterminator needed: ${npcProfile} got a mess of rats in my basement. Bring me 10 rat tails in return for payment. (Listed address has no basement and owner has no rat problem. The advert has a typo and should point to a house further down the way.)`
    },
  },
  "Sunk Ore": {
    type: [PlothookType.Paper],
  ...
  "Sick Granny": {
    ...
    function(town) {
      ...
      return `Sick Grandmother: My ${npcProfile} is sick and needs a shipment of medicine. Our normal courier has gone missing, so we desperately need it delivered. Expedited fee available with a bonus upon delivery (Grandma makes a mean elven bread.)`
    },
  },
  "Squirrel Hunting": {
    type: [PlothookType.Paper],
  ...
  "Magic Bakery": {
    ...
    function(town) {
      ...
      const npcProfile = profile(npc, `We`)
      return `TEST SUBJECT NEEDED! ${npcProfile} at the Barrington Bakery are looking for subjects to test out our new magical breads. We infuse magic into the mixing process for a, hopefully, tasteful bread. You will need to taste each bread, and have our scholar review the side effects. You will be paid per day.`
    },
  },
  "Trophy Arrow": {
    type: [PlothookType.Paper],
    function(town) {
      ...
      const npcProfile = profile(npc, `alchemist`)
      return `Lost Arrow: While practicing with a bow, my son lost a trophy arrow (he was not supposed to use). He was practicing in the Northern plains and thinks the arrow went into the forbidden forest. Please retrieve this arrow. As an ${npcProfile}, I will pay in healing potions.`
    },
  },
  "Goblin Encampment": {
    type: [PlothookType.Paper],
  ...
  "Kobold Flour": {
    ...
    function(town) {
      ...
      const npcProfile = profile(npc, `Local baker`)
      return `Kobold Flour: ${npcProfile} seeks Kobold Flour for the upcoming Monster festival. This specialty item can only be found in Kobold encampments. Will pay <<money 1000>> per pound (maximum of 100 pounds).`
    },
  },
  "Bard Wanted": {
    type: [PlothookType.Paper],
    function(town) {
      ...
      return `${buildingProfile} is looking for a bard to entertain the crowds on Thursday Nights (mug for a copper night).`
    },
  },
  "Armed Escort Needed": {
    type: [PlothookType.Paper],
  ...
  "Burial Escort Needed": {
    ...
    function(town) {
      ...
      return `Burial Escort needed: Our recently deceased father needs to be buried in the family lot, six miles north through the badlands. A small party escort is required in case of trouble. Contact ${profile(
        npc
      )} for details.`
    },
  },
  ...
  "Traveling Bards": {
    ...
    function(town) {
      ...
      return `Troupe of traveling bards need a few strong individuals to help schlep and set up our equipment on our world tour. Can pay in booze and lodging. Please contact ${profile(
        npc
      )} for details.`
    },
  },
  "Door to Door": {
    ...
    function(town) {
      ...
      return `Detail-oriented individuals needed to go door-to-door to market my magical floor-sucker-cleaner. Commission based. Please contact ${profile(
        npc
      )} for details.`
    },
  },
  "Library Guards": {
    ...
    function(town) {
      ...
      return `Book keepers needed! Not really. Bandit-wizards have been trying to steal from my personal library of arcane books. Need tough, preferably illiterate, bodies to defend collection until I can secure safer storage. Please contact ${profile(
        npc
      )} for details.`
    },
  },
  "Destroy The Bell": {
    ...
    function(town) {
      ...
      return `Tower Demolition Sought: The four story bell tower has been deemed by the city council as a stain on the communities reputation, as it was built with ‘ill-gotten’ funds by the rencently disgraced and now former mayor, Cornul Glassen. The council is offering 500 gold to anyone who can reduce the bell tower, which was dedicated in his honor, to rubble without loss of life or other property in the town square. Plans must be approved by council before work can begin. Please contact ${profile(
        npc
      )} for details.`
    },
  },
  ...
  "Mole Dispatch": {
    ...
    function(town) {
      ...
      return `Mole Dispatch – a group of unusually large moles wreck our fields! We are in dire need of a someone who is getting rid of those pests. Payment in gold or products from our farmers market. Please contact ${profile(
        npc
      )} for details.`
    },
  },
  ...
  "Daughter Dying": {
    ...
    function(town) {
      ...
      return `Cleric needed – Daughter Ill and Dying (A local ${profile(
        npc,
        `father`
  ...
  "Kobold Ate It": {
    ...
    function(town) {
      ...
      return `Adventurers needed! Our Kobold in our previous adventuring party is currently missing and is in possession of an ancient and dangerous artifact. Problem is, he ate it. Please return the artifact in tact! The Kobold (Skrazz) can be returned dead, alive, or not at all for all we care. <<money 100000>> Reward. Please contact ${profile(
        npc
      )} for details.`
    },
  },
  "Linguist Needed": {
    ...
    function(town) {
      ...
      return `Multi-Linguist Needed: While going through my pappy’s attic I found this really old ${
        map.tippyWord
      } that I’m pretty sure leads to something cool. The problem is, I can’t read it! Pretty sure it’s Celestial, Draconian or Elvish cuz the letters are all curly. If the map leads to some sweet treasure I’ll share it fairly with you! Please contact ${profile(
        npc,
        `me`
      )} for details.`
    },
  ...
  "Taste Tester": {
    ...
    function(town) {
      ...
      return `Taste-Tester Needed: Lord ${profile(
        npc,
        npc.lastName
  ...
  "Strange Doll": {
    ...
    function(town) {
      ...
      return `FOUND: Stange child’s doll. Blue gingham dress, yellow yarn hair with blue satin bow, made of painted wood. Eyes are large and very lifelike, seeming to follow you around the room. IF YOU RECOGNIZE THIS DOLL PLEASE COME AND TAKE IT I’LL GIVE YOU <<money 500>>. Please contact ${profile(
        npc
      )} for details.`
    },
  },
  ...
  "Who Am I": {
    ...
    function(town) {
      ...
      // #
      return `WHO AM I?: ${profile(npc, `I`)} woke up in a gutter this morning outside of ${profile(
        building,
        ``,
  ...
  "First Class Male": {
    ...
    function(town) {
      ...
      return `FIRST CLASS MALE: If you need a message that needs delivering, look no further than this ${profile(
        npc,
        `First Class Male`
  ...
  "Secret About To Be Revealed": {
    ...
    function(town) {
      ...
      return `My Family is about to find out about my secret. Good liars and problem solvers in need. Please contact ${profile(
        npc
      )} for details.`
    },
  },
  "Bridge Collapsed": {
    ...
    function(town) {
      ...
      const npcProfile = profile(npc)
      return `The bridge collapsed again and I lost my wagon. Will pay good gold if bridge repaired, and more if wagon retrieved. Please contact ${npcProfile} for reward. (Twist: Goblins have been booby trapping the bridge to steal the contents of wagons.)`
    },
  },
  "Nothing Sexual": {
    type: [PlothookType.Paper],
  ...
  "Missing Pet": {
    ...
    function(town) {
      ...
      const npcProfile = profile(npc)
      return `Missing pet! My pet has been missing since three nights ago and has not returned home. If found return to ${npcProfile}, whoever finds him will be rewarded greatly! (Twist: the missing pet is actually a small dragon.)`
    },
  },
  ...
  "Missing Skeletons": {
    ...
    function(town) {
      ...
      const npcProfile = profile(npc)
      return `Missing: One (1) semi-intelligent skeleton. 5 feet 10 inches. Last seen wearing blue cloak. He was sent to the market for some groceries five days ago, and hasn’t come home. His creator misses him very much. <<money 5000>> & four (4) Stones of Farspeech upon return. Please contact ${npcProfile} for details.`
    },
  },
  "Nice Guy": {
    type: [PlothookType.Paper],
    function(town) {
      ...
      const npcProfile = profile(npc)
      return `Love of my life! Please, I need your help to make me look cool in front of the love of my life, so that I can win her heart! I could never hope to win her by normal means since I’m poor. I will give you all of my life savings in return. Bonus if you look scary! Please contact ${npcProfile} for details. //DM note: The plan is to have the players perform as crooks flirting and trying to pick up the girl and in turn be chased away by the contract giver to make him look cool.`
    },
  },
  "Be Your Own Boss": {
    type: [PlothookType.Paper],
    function(town) {
      ...
      const npcProfile = profile(npc)
      return `Need to make silver quick? Why not platinum! Rent doesn’t pay itself ya’ know! Find ${npcProfile} and ask him how you can become your own boss and never have to work a day in your life! (Can be a quest to thwart a predatory pyramid scheme OR be the starting branch in a murder investigation once Raul is found murdered, likely by one of the people he recruited.)`
    },
  },
  "Work From Home": {
    type: [PlothookType.Paper],
    function(town) {
      ...
      const npcProfile = profile(npc)
      return `WORK FROM HOME: Easy money that anyone can do! Finally live your dream lifestyle! Be your own boss! Make thousands in your first week! Retire early! Don’t be left out! Contact ${npcProfile} for details.`
    },
  },
  ...
  "Incredible Opportunity": {
    ...
    function(town) {
      ...
      return `Sales assistant wanted. Incredible pay! Contact ${profile(
        npc
  ...
  "Horsebreaker Needed": {
    ...
    function(town) {
      ...
      return `Horsebreaker needed! Inquire at the Post Office, ask for ${profile(
        npc
  ...
  "Jeweler Needed": {
    ...
    function(town) {
      ...
      return (
        `Do you think you know your gems? A grand test awaits at ${profile(npc, npc.lastName)}'s` +
  ...
  "Just No MIL": {
    ...
    function(town) {
      ...
      return `Help! My mother-in-law is coming to town for a visit and ${profile(
        npc,
        `I`
  ...
  "Lost Hat": {
    ...
    function(town) {
      ...
      return `Help recovering property! This is your local bard, ${profile(
        npc
  ...
  "Birthday Party": {
    ...
    function(town) {
      ...
      return `URGENT: It is my little pookums birthday and the entertainment cancelled! Anyone may apply as replacements at the ${profile(
        npc,
        npc.lastName
      )} estate.`
    },
  ...
  "Lost Cat": {
    ...
    function(town) {
      ...
      return `Help find my lost pet ${
        cat.tippyWord
      }! Reward of <<money 3000>> to anyone who finds him. Contact ${profile(npc)} for details on him.`
    },
  },
  "Bakery Delivery": {
    type: [PlothookType.Paper],
  ...
  "The Painted Devil": {
    ...
    function(town) {
      ...
      const npcProfile = profile(npc, `well dressed elf`)
      return `A ${npcProfile} runs up to the party, desperately shouting something about a painting and a demon.`
    },
  },
  "The Crack": {
    type: [PlothookType.Event],
    function(town) {
      ...
      const factionProfile = profile(faction, ``, `town.factions`)
      return `A huge crack has recently appeared in the center of the town square. ${factionProfile} are keeping everyone away from the fissure, especially since it smells of sulfur and green fumes will occasionally puff out of it.`
    },
  },
  "The Press Gang": {
    type: [PlothookType.Event],
  ...
  "The Merchant Ship": {
    ...
    function(town) {
      ...
      const npcProfile = profile(npc, npc.lastName)
      return `Looking for adventure? Ready to leave this crummy town? Want to earn lots of gold? Ask for Captain ${npcProfile} at the docks.`
    },
  },
  "A Lost Bunny": {
    type: [PlothookType.Paper],
    function(town) {
      ...
      const npcProfile = profile(npc)
      return `Help find my lost rabbit ${bunny.tippyWord}! Reward of <<money 2>> to anyone who finds him. Contact ${npcProfile} for details on him.`
    },
  },
  "Dead or Alive": {
    type: [PlothookType.Paper],
    function(town) {
      ...
      const npcProfile = profile(npc)
      return `‘Wanted Dead or Alive – ${npcProfile}, master assassin! Warning: very heavily armed and dangerous. See attached picture. Reward: <<money 100000>>’`
    },
  },
  "Brimstone!": {
    type: [PlothookType.Paper],
    function(town) {
      ...
      const factionProfile = profile(faction, ``, `town.factions`)
      return `Fire and brimstone! Eternal torture and damnation! That is what awaits those who reject ${factionProfile}!`
    },
  },
  "Experiment Subjects": {
    type: [PlothookType.Paper],
    function(town) {
      ...
      const npcProfile = profile(npc, `spell caster`)
      return `Local ${npcProfile} looking for willing test subjects. <<money 20000>> per experiment. <money 400000>> if you lose a limb.`
    },
  },
