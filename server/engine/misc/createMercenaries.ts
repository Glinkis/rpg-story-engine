...
export function createMercenaries(town: Town) {
  const mercenaries = {
    ...
    attitude: randomValue(ATTITUDES),
    currently: randomValue(CURRENTLYS),
    specializes: randomValue(SPECIALIZATIONS),
    notorious: randomValue(NOTORIETIES),
    tactics: randomValue(TACTICS),
    armour: randomValue(ARMOURS),
    weapon: randomValue(WEAPONS),
  }
